...
export function useOrders() {
  ...
  /**
   * Permanently deletes an order row. There's no "isDeleted" soft-delete
   * flag on orders (unlike products) — this is a hard delete, used by the
   * admin to clear out test/duplicate/cancelled orders from the ledger.
   */
  const deleteOrder = async (id: string) => {
    try {
      ...
      throw new Error(getSupabaseErrorMessage(err, 'Failed to delete order.'));
    }
  };
