...
export default function AdminProducts() {
  ...
  // Lets the admin type a colour that isn't in the preset list (e.g. "Wine",
  // "Peacock Blue") and add it as a selected tag. Duplicate-safe, case-insensitive.
  ...
  const handleCustomColorKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    // Prevent Enter from submitting the whole "Add Saree" form.
    if (e.key === 'Enter') {
      e.preventDefault();
      addCustomColor();
    }
  ...
  // Same idea for occasions (e.g. "Engagement", "Haldi").
  const addCustomOccasion = () => {
    const value = customOccasionInput.trim();
    if (!value) return;
    setSelectedOccasions((prev) =>
      prev.some((o) => o.toLowerCase() === value.toLowerCase()) ? prev : [...prev, value]
    );
    setCustomOccasionInput('');
  };

  const handleCustomOccasionKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      addCustomOccasion();
    }
          ...
          <div className="bg-[#FDF8F2] border-2 border-[#B8860B] rounded-lg max-w-lg w-full p-5 sm:p-7 shadow-2xl relative overflow-y-auto max-h-[92vh]">
            ...
            <form onSubmit={handleFormSubmit} className="flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="flex flex-wrap gap-2">
                  {Array.from(new Set([...availableColors, ...selectedColors])).map((color) => {
                    const active = selectedColors.includes(color);
                    const isCustom = !availableColors.includes(color);
                    return (
                      <button
                        key={color}
                        type="button"
                        onClick={() => toggleColor(color)}
                        title={active ? 'Click to remove' : 'Click to select'}
                      ...
                      >
                        {color}
                        {isCustom && active && <X className="h-3 w-3" />}
                      </button>
                    );
                  })}
                </div>
                <div className="flex gap-2 mt-1">
                  <input
                    ...
                    placeholder="Add a custom colour (e.g. Wine, Peacock Blue)"
                    className="flex-1 bg-[#FDF8F2] border border-[#B8860B]/25 rounded px-3 py-1.5 text-xs focus:border-[#7A1C2E] focus:outline-none"
                  />
                  <button
                    type="button"
                    onClick={addCustomColor}
                    className="shrink-0 bg-[#B8860B]/15 hover:bg-[#B8860B]/25 text-[#7A1C2E] px-3 py-1.5 rounded text-xs font-bold uppercase"
                  >
                    Add
                  </button>
                </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="flex flex-wrap gap-2">
                  {Array.from(new Set([...OCCASION_OPTIONS, ...selectedOccasions])).map((occasion) => {
                    const active = selectedOccasions.includes(occasion);
                    const isCustom = !OCCASION_OPTIONS.includes(occasion);
                    return (
                      <button
                        key={occasion}
                        type="button"
                ...
                <div className="flex gap-2 mt-1">
                  <input
                    ...
                    placeholder="Add a custom occasion (e.g. Engagement, Haldi)"
                    className="flex-1 bg-[#FDF8F2] border border-[#B8860B]/25 rounded px-3 py-1.5 text-xs focus:border-[#7A1C2E] focus:outline-none"
                  />
                  <button
                    type="button"
