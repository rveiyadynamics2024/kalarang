...
import { ShoppingBag, PhoneCall, Check, Clock, Truck, ShieldAlert, CheckCircle2, Trash2 } from 'lucide-react';

export default function AdminOrders() {
  const { subscribeToOrders, updateOrderStatus, deleteOrder } = useOrders();
  ...
  const handleDeleteOrder = async (order: Order) => {
    if (
      !window.confirm(
        `Permanently delete this order from ${order.customerName} (₹${order.total.toLocaleString('en-IN')})? This cannot be undone.`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      // No need to manually update local state — the realtime subscription
      // (and 15s poll fallback) will refresh the list automatically.
    } catch (err) {
      console.error('Order delete failure:', err);
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-gray-100 pb-3">
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <button
                      ...
                      title="Delete order"
                      className="inline-flex items-center justify-center h-8 w-8 rounded border border-red-200 text-red-600 hover:bg-red-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors cursor-pointer shrink-0"
                    >
