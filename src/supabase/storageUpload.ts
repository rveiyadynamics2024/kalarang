...
async function compressImage(file: File): Promise<Blob> {
  ...
  // Some browsers can't decode certain phone-camera formats (e.g. iPhone
  // .HEIC/.HEIF, some raw JPEGs with unusual EXIF orientation) via
  // createImageBitmap. Previously that threw and killed the whole upload.
  // Now we just skip compression and upload the original file instead.
  try {
    ...
    console.warn(`Could not compress "${file.name}", uploading original file instead.`, err);
    return file;
  }
}
...
export async function uploadFile(file: File, options: UploadOptions): Promise<string> {
  ...
  // Only enforce the tighter 2MB "compressed" cap when compression actually
  // ran and produced a new blob. If compression was skipped/failed and we
  // fell back to the original file, the earlier maxSizeMb (8MB) check already
  // covers it — re-checking against the 2MB cap here would reject perfectly
  // uploadable photos just because the browser couldn't compress them.
  const wasCompressed = compressed !== (file as unknown as Blob);
  if (!isVideo && wasCompressed && compressed.size > MAX_COMPRESSED_BYTES) {
    throw new Error(
